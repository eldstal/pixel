...
FULLSCREEN_START = 1 # sdl only

DEFINES = -DPIXEL_WIDTH=$(PIXEL_WIDTH) -DPIXEL_HEIGHT=$(PIXEL_HEIGHT) -DPORT=$(PORT) -DUDP_PROTOCOL=$(UDP_PROTOCOL)
...
LDFLAGS_SDL = `pkg-config --libs sdl2` -lm -lpthread -O3 -fsanitize=address -flto -g -fomit-frame-pointer
CFLAGS_SDL = -Wall -Wextra -Werror -pedantic -std=c11 -c `pkg-config --cflags sdl2` -Wall -O3 -fsanitize=address -march=native -flto -g -fomit-frame-pointer
